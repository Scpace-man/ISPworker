...
{
    $db->query("UPDATE biz_rechnungtodo SET `kuendigen_zum`='0000-00-00' WHERE `posid` =".$_REQUEST['posid']);
}
...
if($_REQUEST[saveeditinvoice]=="true") {
  ...
  $db->query("update biz_rechnungen set positionen='$_REQUEST[positionen]', mwst='$_REQUEST[rechmwst]' where kundenid='$_REQUEST[kundenid]' and rechnungid='$_REQUEST[rechnungid]'");

  if($_REQUEST["bool_rechneueid"]=="1") {
    $res = $db->query("select * from biz_rechnungen where kundenid='$_REQUEST[kundenid]' and rechnungid='$_REQUEST[rechnungid]'");
    $row = $db->fetch_array($res);
    
    $res = $db->query("insert into biz_rechnungen (kundenid,anschrift,positionen,datum,profilid,mwst,kommentar,status,mahndatum) 
		       values ('$row[kundenid]','$row[anschrift]','$row[positionen]','$_REQUEST[datum]','$row[profilid]','$row[mwst]','$row[kommentar]','$row[status]','$row[mahndatum]')");
  
    pdfinvoice($db->insert_id());
    send_invoice($db->insert_id(), $_REQUEST[kundenid], $template="std_neuerechnung");
  }
  
  if($_REQUEST["bool_rechstornieren"]=="1") {
    $db->query("update biz_rechnungen set status='storniert' where kundenid='$_REQUEST[kundenid]' and rechnungid='$_REQUEST[rechnungid]'");
    pdfinvoice($_REQUEST["rechnungid"]);
    send_invoice($_REQUEST[rechnungid], $_REQUEST[kundenid], $template="std_stornierterechnung");
  }
}
...
<?
$res = $db->query("SELECT r.posid,r.beginnabrechnung,r.produktanzahl,r.kuendigen_zum,r.produktid,r.produktkommentar,r.profilid,
		    p.bezeichnung,p.preis,p.abrechnung
...
<tr>
  ...
  $kuendigen = explode("-", $row['kuendigen_zum']);
    if((count($kuendigen) == 3) and $row['kuendigen_zum'] != "0000-00-00")
    {
...
<tr>
  ...
  <textarea name="positionen" style="width:670; height:150;" wrap=off>
  <?=$row[positionen]?>
  </textarea>
  </td>
</tr>
<tr>
  <td bgcolor="#ffffff"><input type="text" name="rechmwst" value="<?=$row["mwst"]?>" size="2">% MwSt. (-1 für nicht definiert.)</td>
</tr>
<tr>
  <td bgcolor="#ffffff"><input type="checkbox" name="bool_rechneueid" value="1"> Neue Rechnungsnummer vergeben und Rechnung erzeugen</td>
</tr>
<tr>
  <td bgcolor="#ffffff"><input type="checkbox" name="bool_rechstornieren" value="1"> Alte Rechnung stornieren</td>
</tr>
<tr>
  <td bgcolor="#ffffff"><input type="text" name="datum" value="<?=date("Y-m-d");?>" size="10""> YYYY-MM-DD Datum, falls neue neue Rechnung</td>
</tr>
...
<td width="16" bgcolor="#ffffff" valign="top"><a href="module/biz/rechnung_show.php?rechnungid=<?=$row[rechnungid]?>" target="invoicewin"><img src="img/pdf.gif" border="0"></a>
